...
class Align_Depth_Eye_Track():
    ...
    def main(self):
        ...
        self.total = np.array([x_Coord,y_Coord,z_Coord])
        
        self.debug_image = cv2.cvtColor(self.debug_image, cv2.COLOR_BGR2RGB)
        # 3D plot
    ...
    def plotCoord(self, total, eye):
        fig = plt.figure(figsize=(75.59,42.52))
        ax = fig.add_subplot(121, projection='3d')
        ax.set_xlim3d(-300,300) # x_coord
        ax.set_ylim3d(0,600) # z_coord
        ax.set_zlim3d(-300,300) # y_coord
        ax.view_init(15,290)
        
        ax.plot(total[0,:].reshape(-1,),total[2,:].reshape(-1,),-total[1,:].reshape(-1,),'yo',markersize=0.1)
        ax.plot(eye[0],eye[2],-eye[1],'bo',markersize=3)
